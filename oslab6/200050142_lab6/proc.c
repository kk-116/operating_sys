...
void exit(void)
{
  ...
  {
    curproc->runtime += (ticks - curproc->time);
  }
  curproc->time = ticks;
...
void yield(void)
{
  ...
  if(myproc()->pid % 2 == 1 || myproc()->used == 0){
    acquire(&ptable.lock); // DOC: yieldlock
    if (myproc()->state == RUNNING)
    {
      myproc()->runtime += (ticks - myproc()->time);
    }
    myproc()->time = ticks;
