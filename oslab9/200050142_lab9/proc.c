...
block_sem(void* chan)
{
  ...
  acquire(&ptable.lock);
  uint phya,virta;
  virta = (uint) PGROUNDDOWN((uint)chan);
  phya = (uint) uva2ka(myproc()->pgdir,(char*)virta);
  phya += (uint)chan - virta;
  release_lock(chan);
  sleep((void*)phya, &ptable.lock);
  release(&ptable.lock);
  acquire_lock(chan);
}
...
These are generally used to obtained page aligned address.
PGROUNDUP will round the address to the higher multiple of PGSIZE while PGROUNDDOWN will round it to the lower multiple of PGSIZE.
*/
