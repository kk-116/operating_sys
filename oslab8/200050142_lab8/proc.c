...
// Copy process data to the new thread
  np->state = UNUSED;
  np->tgid = p->pid;  //grp leader = this process
  np->sz = p->sz;
  np->parent = p;
  np->pgdir = p->pgdir;
  *np->tf = *p->tf;
  ...
  sret = stack + PGSIZE - sizeof(void *) -sizeof(int *) ;
  *(uint*)sret = 0xFFFFFFF;

  // Push first argument to the stack of thread
  sarg = stack + PGSIZE - sizeof(int *);
  *(uint*)sarg = (uint)arg;
  ...
  np->tf->esp += PGSIZE -sizeof(void*) - sizeof(int*);
  np->tf->ebp = np->tf->esp;
...
join()
{
  ...
  for(;;){
    ...
    havekids = 0;
    for(p = ptable.proc; p < &ptable.proc[NPROC]; p++){
      if(p->tgid != curproc->pid || p->parent != curproc)
        continue;
      havekids = 1;
      if(p->state == ZOMBIE){
        // Found one.
