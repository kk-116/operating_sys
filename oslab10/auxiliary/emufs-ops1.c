...
int open_root(int mount_point)
{
    ...
    */
    ...
    for (i = 0; i < MAX_DIR_HANDLES; i++)
    {
        if (dir[i].mount_point == -1)
        {
            read_inode(dir[i].mount_point, dir[i].inode_number, &in);
            printf("%d\n",in.parent);
            if (in.parent == 255)
                return i;
        }
...
int change_dir(int dir_handle, char *path)
{
    /*
        ...
        * Return value: -1,		error
                         1, 	success
    */
    int in_ch = return_inode(dir[dir_handle].mount_point, dir[dir_handle].inode_number, path);
    if (in_ch == -1)
        return -1;
    dir[dir_handle].inode_number = in_ch;
    return 1;
}
...
int emufs_delete(int dir_handle, char *path)
{
    ...
    */
    ...
    int parent_node = delete_entity(dir[dir_handle].mount_point, inode);
    struct inode_t in;
    read_inode(dir[dir_handle].mount_point, parent_node, &in);
    int i;
    for (i = 0; i < 4; i++)
    {
        if (in.mappings[i] == inode)
            break;
    }
    for (i = i + 1; i < 4; i++)
    {
        in.mappings[i - 1] = in.mappings[i];
    }
    in.mappings[3] = -1;
    write_inode(dir[dir_handle].mount_point, parent_node, &in);
}
...
int open_file(int dir_handle, char *path)
{
    ...
    */
    //    struct inode_t* in;
    int innum = return_inode(dir[dir_handle].mount_point, dir[dir_handle].inode_number, path);
    int fnum = alloc_file_handle();
    if (fnum == -1)
        return -1;
    files[fnum].mount_point = dir[dir_handle].mount_point;
    files[fnum].inode_number = innum;
    files[fnum].offset = 0;
    return 1;
}
