	...
	*/
	...
	if(superblock->used_inodes < MAX_INODES){
       for(int i=0;i<MAX_INODES;i++){
		...
		printf("Sorry there are no free inodes!!!\n");
		return -1;
	}
...
void free_inode(int mount_point, int inodenum){
	...
	*/
	...
	read_superblock(mount_point,superblock);
       ...
       if(inodenum<MAX_INODES && inodenum >= 0){
		if(superblock->inode_bitmap[inodenum] == 1){
           superblock->used_inodes -=1;
		   superblock->inode_bitmap[inodenum] == 0;
		   write_superblock(mount_point,superblock);
		}
		else{
			printf("The inode is already free!!!\n");
		}
	  }
}
...
void read_inode(int mount_point, int inodenum, struct inode_t *inodeptr){
	...
	*/
	...
	if(inodenum>15 && inodenum<MAX_INODES){
		blocknum =2;
	}
	char buf[BLOCKSIZE];
	struct metadata_t meta;
	//printf("%d device fd\n",mounts[mount_point].device_fd);
	readblock(mounts[mount_point].device_fd,blocknum,buf);
    // meta = *(struct metadata_t*)buf;
	// inodeptr = &meta.inodes[inodenum%16] ;
	memcpy(&meta,buf,BLOCKSIZE);
	memcpy(inodeptr,&meta.inodes[inodenum],sizeof(struct inode_t));
    //printf("read the inode successfully\n");
...
void write_inode(int mount_point, int inodenum, struct inode_t *inodeptr){
	...
	*/
	...
	char buf[BLOCKSIZE];
	char wr_buf[BLOCKSIZE];
	struct metadata_t meta;
	readblock(mounts[mount_point].device_fd,blocknum,buf);
    // meta = *( struct metadata_t*)buf;
	// meta.inodes[inodenum%16] = *inodeptr;
    // memcpy(wr_buf, &meta, sizeof(struct metadata_t));

	memcpy(&meta,buf,sizeof(struct metadata_t));
	memcpy(&meta.inodes[inodenum],inodeptr,sizeof(struct inode_t));
	writeblock(mounts[mount_point].device_fd,blocknum,(void*)&meta);
...
int alloc_datablock(int mount_point){
	...
	*/
    struct superblock_t *superblock = malloc(sizeof(struct superblock_t));
	read_superblock(mount_point,superblock);
	if(superblock->used_blocks < MAX_BLOCKS){
       for(int i=0;i<MAX_BLOCKS;i++){
		if(superblock->block_bitmap[i] == 0){
           superblock->used_blocks +=1;
...
void free_datablock(int mount_point, int blocknum){
	...
	*/
	...
	read_superblock(mount_point,superblock);
       ...
       if(blocknum<MAX_BLOCKS && blocknum >= 3){
		if(superblock->block_bitmap[blocknum] == 1){
           superblock->used_blocks -=1;
