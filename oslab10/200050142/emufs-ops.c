...
int open_root(int mount_point)
{
    ...
    */
    for (int i = 0; i < MAX_DIR_HANDLES; i++)
    {
        if (dir[i].mount_point == mount_point)
        {
            return i;
        }
    }
...
int emufs_create(int dir_handle, char *name, int type)
{
    ...
    */
    ...
    read_inode(dir[dir_handle].mount_point, dir[dir_handle].inode_number, pinode);
    int i;
    for (i = 0; i < 4; i++)
    {
        if (pinode->mappings[i] == -1)
            break;
        if (i == 3)
            return -1;
    }
    ...
    int inodenum = alloc_inode(mount_point);
    struct inode_t *inode;
    read_inode(mount_point, inodenum, inode);
    inode->parent = dir[dir_handle].inode_number;
    pinode->mappings[i] = inodenum;
    inode->type = type;
    if (type == 1)
    {
        for (int i = 0; i < MAX_DIR_HANDLES; i++)
        {
            int mount_point = dir[i].mount_point;
            int inodenum = alloc_inode(mount_point);
            struct inode_t *inode;
            read_inode(mount_point, inodenum, inode);
            inode->parent = dir[dir_handle].inode_number;
    ...
    else if (type == 0)
    {
        for (int i = 0; i < MAX_FILE_HANDLES; i++)
        {
            int mount_point = files[i].mount_point;
            int inodenum = alloc_inode(mount_point);
            struct inode_t *inode;
            read_inode(mount_point, inodenum, inode);
            inode->parent = files[dir_handle].inode_number;
            pinode->mappings[i] = inodenum;
            inode->type = type;
            int c = 0;
            while (name[0] != 0)
...
int emufs_read(int file_handle, char *buf, int size)
{
    ...
    */
    ...
    read_inode(mount_point, inodenum, inodeptr);
    int blocknum = inodeptr->mappings;
    int fsize = size;
    while (fsize > 0)
    {
        char buffer[BLOCKSIZE];
        struct metadata_t meta;
        read_datablock(mount_point, blocknum, buffer);
